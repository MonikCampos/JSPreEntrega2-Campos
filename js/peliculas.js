    ...
    movies.forEach(movie => {
      ...
      <div class="media border p-3">
        <img src="${movie.image}" alt="${movie.title}" class="mr-3 mt-3">
          ...
          <a onclick="showMovie(${movie.id});"><h3>Sinopsis de la Película <i class="fa-sharp fa-solid fa-circle-play"></i></h3></a>
          </div>
      </div>
      `;
...
function sinopsisMovie (id) {
  ...
  if (foundMovie) {
    ...
    movies.forEach(movie => {
      let contenedorMovie = document.createElement('div');
      contenedorMovie.classList.add('container');
      contenedorMovie.innerHTML = `
      <div class="row">
        <div class="col-lg-8">
          <div class="movie-info-box">
            <h2 class="name">${movie.qualification}<br><strong>${movie.title}</strong></h2>
              <ul class="features">
                  <li>
                      <div class="year">${movie.minutes} minutos.</div>
                  </li>
                  <li>
                      <div class="hd">${movie.qualification} <b>${movie.origen}</b></div>
                  </li>
                  <li>
                      <div class="tags">${movie.gender}</div>
                  </li>
              </ul>
              <p class="description">${movie.descripcion}</p>
                  <a href="">ENTRADAS</a>
              <div class="rate-box">
                  <a href=""><i class="lni lni-calendar"></i></a>
              </div>
              
              
              <div class="block mt-5 video-player video-container">
                  <iframe src="${movie.movie}" frameborder="0" allowfullscreen></iframe>
              </div>
      ...
      <div class="col-lg-4">
          <div class="movie-side-info-box">
              <figure>
                  <img src="${movie.image}" alt="Image"></figure>
              <ul>
                  <li><strong>Título Original: </strong>${movie.title}</li>
                  <li><strong>Orígen:</strong>${movie.origen}</li>
                  <li><strong>Género:</strong>${movie.gender}</li>
                  <li><strong>Director:</strong>${movie.director}</li>
                  <li><strong>Calificación:</strong>${movie.qualification}</li>
                  <li><strong>Duración:</strong>${movie.minutes}</li>
              </ul>
          </div>
      </div>
  </div>
